...
class MyOrderView(LoginRequiredMixin,ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["active_orders"] = Order.objects.filter(user=self.request.user, is_active=True)
        context["inactive_orders"] = Order.objects.filter(user=self.request.user, is_active=False)
        return context
...
class CreateOrderProductView(LoginRequiredMixin,CreateView):
    ...
    def form_valid(self, form):
        ...
        if order_item:
            form.instance.quantity = order_item.quantity + 1  # Si existe, aumentar cantidad
            order_item.delete() 
        else:
